...
/* Record page timestamps for each process */
...
static void allscore() { 
    ...
    //         if(page == MAXPROCPAGES - 1) {
    //             fprintf(fp, "%8d", timestamps[proc][page]); // if final row element, don't print trailing comma
    //         } else {
    //             fprintf(fp, "%8d,", timestamps[proc][page]);
    //         }
    //     }
    ...
    FILE *fp = fopen("transitions2.csv", "w");

    // print the transitions matrix in csv format
    for(int proc = 0; proc < MAXPROCPAGES; proc++) {
        // printf("\n%d :", proc);
        for(int page = 0; page < MAXPROCPAGES; page++) {
            if(page == MAXPROCPAGES - 1) {
                fprintf(fp, "%8d", transitions[proc][page]); // if final row element, don't print trailing comma
            } else {
                fprintf(fp, "%8d,", transitions[proc][page]);
            }
        }
        fprintf(fp, "\n");
    }
